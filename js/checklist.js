...
(function () {
  ...
  // evita doble binding si se carga más de un script
...
function track(event, params = {}) {
  try {
    if (window.gtag) { gtag('event', event, params); return; }
    if (window._paq) { window._paq.push(['trackEvent', 'Checklist', event, JSON.stringify(params)]); return; }
...
function showStepErrors(messages) {
  ...
  if (!box) return;
  if (!messages.length) { box.textContent = ''; box.hidden = true; return; }
  box.hidden = false;
...
function validateStep(idx) {
  ...
  if (idx === 0) {
    ...
    if (!nombre.value.trim()) { setFieldError(nombre, 'El nombre es obligatorio.'); msgs.push('Completa tu nombre.'); }
    if (!isEmail(email.value)) { setFieldError(email, 'Introduce un email válido.'); msgs.push('El email no tiene formato válido.'); }
    if (!obj.value.trim())     { setFieldError(obj, 'Cuéntame tu objetivo principal.'); msgs.push('Escribe tu objetivo principal.'); }
  } else if (idx === 1) {
    const anySelected =
...
document.querySelectorAll('input[type="checkbox"], input[type="radio"]').forEach(el => {
  el.addEventListener('change', () => track('field_toggle', {
    name: el.name, value: el.value, checked: el.checked || el.matches(':checked')
  }));
});
...
if (form) {
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const snapshot = new FormData(form);      const resp = await fetch(form.action, {
        method: 'POST',
      ...
      if (successBox) {
        ...
        try {
          const K_VIEW = 'ck_views', K_SEND = 'ck_sends';
          const prevSends = +localStorage.getItem(K_SEND) || 0; localStorage.setItem(K_SEND, String(prevSends + 1));
          const views = +localStorage.getItem(K_VIEW) || 1; // evita división por 0
          const sends = +localStorage.getItem(K_SEND) || 0;
          const rate = ((sends / views) * 100).toFixed(1);
          // Evitar duplicados
          if (!successBox.querySelector('[data-conv="1"]')) {
            const p = document.createElement('p');
            p.className = 'muted';
            p.dataset.conv = '1'; p.textContent = `Conversión estimada del checklist: ${rate}% (envíos: ${sends} / vistas: ${views})`; successBox.appendChild(p);
          }
        } catch { }
      ...
      if (again) {
        again.onclick = () => {
          ...
          form.querySelectorAll('fieldset.step, .wizard-actions').forEach(el => el.hidden = false);
